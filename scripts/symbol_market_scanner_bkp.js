    ...
    */
    ...
    function detectDataChange(scanType) {
        ...
        switch (scanType) {
            case 'auto':
                ...
                result.reason = result.hasChanged ? 'Data changed' : 'Data unchanged - skipping to save API cost';
                break;
            ...
            case 'alert-triggered':
                if (!result.hasChanged) {
                    result.shouldSend = false;
                    result.reason = 'Alert fired but market data unchanged (too early or no movement)';
                } else {
                    result.shouldSend = true;
    ...
    function analyzeMarketSentiment(allCoins) {
        ...
        allCoins.forEach((coin) => {
            if ((coin.netTrend || 0) > 40) sentiment.bullish++;
            else if ((coin.netTrend || 0) < -40) sentiment.bearish++;
    ...
    function buildFinalPayload(historyEntry, scanType, dataChangeInfo) {
        try {
            ...
            let institutionalPulse = null;

            if (scanType === 'alert-triggered') {
                institutionalPulse = gatherInstitutionalPulse();

                if (!institutionalPulse) {
                    console.warn('[Payload] ⚠️ Alert-triggered scan but no pulse data available - dropping institutionalPulse from payload');
                } else {
                    console.log('[Payload] ✅ Including institutional pulse data');
                }
            }

            const scanContext = {
                type: scanType,
                triggeredBy: scanType === 'alert-triggered' && window.batchedAlerts ? {
                    alertCount: window.batchedAlerts.length,
                    firstAlert: window.batchedAlerts[0] ? {
                        ticker: window.batchedAlerts[0].asset.ticker,
                        time: window.batchedAlerts[0].signal.timestamp
                    } : null
                } : null
            };
            ...
            const payload = {
                id: `scan_${Date.now()}`, // Explicit ID generation
                trigger: scanType, // Backend expects 'trigger' property
                timestamp: historyEntry.timestamp,
                results: [ // Flatten all unfiltered opportunities for the backend
                    ...historyEntry.unfilteredOpportunities.buys,
                    ...historyEntry.unfilteredOpportunities.sells,
                    ...historyEntry.unfilteredOpportunities.retraces
                ],
            ...
            if (institutionalPulse && institutionalPulse.alerts && institutionalPulse.alerts.length > 0) {
                payload.institutionalPulse = institutionalPulse;
                console.log('[Payload] ✅ institutionalPulse included in payload');
            } else {
                console.log('[Payload] ⏭️ institutionalPulse NOT included (no data or not alert-triggered)');
            }
        ...
        try {
            ...
            if (scanType === 'auto') {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (response) => {
                        console.log(`✅ Alert-triggered scan sent: ${response.status}`);
                        if (response.responseText) {
                            console.log('📥 Response:', response.responseText);
                        }
                    ...
                    onerror: (error) => {
                        console.error('❌ Alert-triggered scan failed:', error);
                    },
                    ontimeout: () => {
                        console.error('⏱️ Alert-triggered scan timeout');
                    },
                    timeout: 10000
                });
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                GM_xmlhttpRequest({
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    onload: (response) => {
                        console.log(`✅ Manual trigger sent: ${response.status}`);
                        if (response.responseText) {
                            console.log('📥 Response:', response.responseText);
                        }
                    ...
                    onerror: (error) => console.error('❌ Manual trigger failed:', error),
                    ontimeout: () => console.error('⏱️ Manual trigger timeout'),
                    timeout: 10000,
                });
    ...
    function startAlertMonitor() {
        setInterval(() => {
            if (window.triggerScreenerScan && window.batchedAlerts) {
                console.log(`\n${'═'.repeat(60)}`);
                console.log(`🚨 ALERT-TRIGGERED SCAN DETECTED`);
                console.log(`   Alerts in batch: ${window.batchedAlerts.length}`);
                window.batchedAlerts.forEach((alert, i) => {
                ...
                window.triggerScreenerScan = false;
