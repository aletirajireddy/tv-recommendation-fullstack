...
(function () {
    ...
    // ═══════════════════════════════════════════════════════════════
    const CONFIG = {
        BACKEND_ENDPOINT: 'http://localhost:3000/institutional-pulse',
    ...
    function syncBatchToBackend(batchData) {
        if (!CONFIG.ENABLE_BACKEND_SYNC) {
            if (CONFIG.DEBUG_MODE) {
                console.log('[Sync] ⏭️ Backend sync disabled');
            }
            return;
        }

        console.log(`[Sync] 📦 Syncing batch to backend (${batchData.alerts.length} alerts)...`);
        ...
        GM_xmlhttpRequest({
            ...
            onload: (response) => {
                ...
                if (CONFIG.DEBUG_MODE) {
                    console.log(`[Sync] ✅ Batch synced successfully: ${response.status}`);
                    console.log(`[Sync] 📊 Sent: ${batchData.alerts.length} alerts, ${batchData.macro_windows.length} windows`);
                }
            },
            onerror: (error) => {
            ...
            ontimeout: () => {
                pulseStats.backendErrors++;
                console.warn('[Sync] ⏱️ Backend sync timeout (10s exceeded)');
            },
            timeout: 10000
        });
    }
