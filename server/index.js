...
app.post('/scan-report', (req, res) => {
    ...
    try {
        ...
        // --- TRIGGER ALERTS (Write Path Only) ---
        // Generate Standard 24h Analytics for Strategy Engine
        const standardAnalysis = generateAnalytics(24);

        // Push to Telegram (Side Effect)
        TelegramService.syncStrategies(standardAnalysis.recommendations);
        // ----------------------------------------
...
// --- HELPER: Generate Analytics & Strategies ---
function generateAnalytics(hours = 24, refTime = null) {
    ...
    pulses.forEach(p => {
        ...
        if (!timeMap.has(timeStr)) {
            timeMap.set(timeStr, { count: 0, coins: new Set(), bullish: 0, bearish: 0, timestamps: [], events: [], momAcc: 0 });
        }
        ...
        let rawMom = 0;
        if (payload.signal && payload.signal.momentum_pct !== undefined) rawMom = parseFloat(payload.signal.momentum_pct);
        else if (payload.asset && payload.asset.momentum !== undefined) rawMom = parseFloat(payload.asset.momentum);

        // Safety check for NaN
        rawMom = isNaN(rawMom) ? 0 : rawMom;
        group.momAcc += rawMom;

        const pTime = date.format('HH:mm:ss');
        group.events.push(`${ticker} ${pTime}`);

        if (!signalsMap.has(ticker)) signalsMap.set(ticker, { ticker, count: 0, sumMom: 0, biasScore: 0, lastTime: pTime });
    ...
    const signals = Array.from(signalsMap.values()).map(s => ({
        ticker: s.ticker,
        x: parseFloat((s.count > 0 ? s.sumMom / s.count : 0).toFixed(2)),
        y: Math.min(s.count * 10, 100),
        bias: s.biasScore > 0 ? 'BULLISH' : (s.biasScore < 0 ? 'BEARISH' : 'NEUTRAL'),
        volSpike: s.count > 5
    }));

    const bursts = [];
    const spreadAnalysis = Array.from(timeMap.entries()).map(([keyTimeStr, d]) => {
        d.timestamps.sort((a, b) => (dayjs(a).valueOf() - dayjs(b).valueOf()));
        ...
        return {
            time: fullTimeStr, full_ts: d.timestamps[0], count: d.count, bullish: d.bullish, bearish: d.bearish,
            unique_coins: unique, spread: spreadStr, density: density.toFixed(2),
            cluster: d.count > 3 ? (d.count > 8 ? 'BURST' : 'STEADY') : 'ISOLATED',
            bias: d.bullish > d.bearish ? 'BULL' : (d.bearish > d.bullish ? 'BEAR' : 'NEUTRAL'),
            si: `${d.bullish}/${d.count}`, mon_pct: (d.momAcc / (d.count || 1)).toFixed(2) + '%',
    ...
    if (spreadAnalysis.length > 0) {
        ...
        if (avgMood > 40) {
            recommendations.push({
                id: 'STRAT_TREND_BULL', type: 'trend', confidence: 'high', title: 'Strong Bullish Momentum',
                description: `Market sentiment is at +${avgMood.toFixed(0)}%. Trend-following systems active.`, tickers: []
            });
        } else if (avgMood < -40) {
            recommendations.push({
                id: 'STRAT_TREND_BEAR', type: 'risk', confidence: 'high', title: 'Bearish Pressure Alert',
                description: `Market sentiment is at ${avgMood.toFixed(0)}%. Caution advised.`, tickers: []
            });
        }
    }
    ...
    if (bursts.length > 0) {
        ...
        recommendations.push({
            id: `STRAT_BURST_${topBurst.full_ts}`, type: 'opportunity', confidence: 'max', title: 'Institutional Activity Spike',
    ...
    if (latestScan) {
        const highScopeAssets = db.prepare(`SELECT ticker, raw_data_json FROM scan_entries WHERE scan_id = ? AND status = 'PASS' ORDER BY json_extract(raw_data_json, '$.score') DESC LIMIT 5`).all(latestScan.id);
        if (highScopeAssets.length > 0) {
            recommendations.push({
                id: 'STRAT_HIGH_SCOPE', type: 'opportunity', confidence: 'high', title: 'High-Quality Setups',
                description: `${highScopeAssets.length} assets passed all filters with strong confluence metrics.`,
                tickers: highScopeAssets.map(a => { const raw = JSON.parse(a.raw_data_json || '{}'); return { ticker: a.ticker, bias: raw.netTrend > 0 ? 'LONG' : 'SHORT' }; })
            });
        }
    }
    ...
    if (spreadAnalysis.length > 0) {
        ...
        if (hasChop) {
            recommendations.push({
                id: 'STRAT_CHOP_WARNING', type: 'info', confidence: 'medium', title: 'Low Conviction Environment',
                description: 'Mixed signals with no clear directional bias. Reduce position sizes.', tickers: []
            });
        }
    }
...
app.get('/api/analytics/pulse', (req, res) => {
    try {
        ...
        // READ ONLY: Genereate Analytics but DO NOT sync to Telegram
        const data = generateAnalytics(hours, refTime);
