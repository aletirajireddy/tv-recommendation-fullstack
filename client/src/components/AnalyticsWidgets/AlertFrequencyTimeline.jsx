...
export function AlertFrequencyTimeline() {
    ...
    const { chartData, timezones } = useMemo(() => {
        if (!analyticsData?.time_spread || analyticsData.time_spread.length === 0) {
            return { chartData: [], timezones: [] };
        }

        // Sort ascending (oldest → newest) for proper left-to-right chart rendering
        const sorted = [...analyticsData.time_spread].sort(
            ...
            <div style={{ width: '100%', height: '220px' }}>
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                        ...
                        {timezones.map((tz, i) => (
                            <ReferenceArea key={i} x1={tz.start} x2={tz.end} fill={tz.fill} strokeOpacity={0} ifOverflow="hidden" />
                        ))}

                        <Bar dataKey="bull_count" stackId="a" fill="#10B981" barSize={8} radius={[0, 0, 4, 4]} isAnimationActive={false} />
                        <Bar dataKey="bear_count" stackId="a" fill="#EF4444" barSize={8} radius={[4, 4, 0, 0]} isAnimationActive={false} />

                        {/*
                         * Recharts Brush: desktop drag handles.
                         * travellerWidth=18 for larger desktop grab-zone.
