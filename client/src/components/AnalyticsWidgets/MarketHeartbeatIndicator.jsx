...
export function MarketHeartbeatIndicator() {
    const { timeline, analyticsData } = useTimeStore();
    ...
    const handleBrushChange = (newRange) => {
        if (newRange && newRange.startIndex !== undefined) {
            ...
            localStorage.setItem('tv_heartbeatBrush', JSON.stringify({ startIndex: newRange.startIndex, endIndex: newRange.endIndex }));
        }
    };
