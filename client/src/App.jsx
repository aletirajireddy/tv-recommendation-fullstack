...
// These four components are always imported eagerly but only RENDER conditionally.
// Eagerly importing them inflates the initial JS bundle by ~30KB+ (Floating*
// pull in their own date pickers / sliders / modal logic). Converting to
// React.lazy ships them in separate chunks that only download when their
// `showPlayback` / `isMonitorModalOpen` / `showThemeBuilder` triggers fire.
// Zero behaviour change — they still mount the same way.
...
function App() {
  ...
  return (
    <div className={styles.appContainer}>
      ...
      {/* OVERLAYS & FLOATING — lazy-loaded, only download chunks when shown */}
      <SelectionDrawer />
      {showPlayback && (
